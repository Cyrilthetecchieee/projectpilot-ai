...
import type { AgentRun, Project, Risk, Task, TestCase } from '../types'
...
const runRecord = (agent: InitializationAgent, trigger: string, startedAt: string, summary: string, status: AgentRun['status'] = 'Completed'): AgentRun => ({ id: `run-${Date.now()}-${agent}`, agent: labels[agent], action: trigger, status, duration: `${((Date.now() - Date.parse(startedAt)) / 1000).toFixed(1)}s`, createdAt: 'Just now', model: 'Simulation', provider: 'Local Mock', trigger, startedAt, completedAt: new Date().toISOString(), summary })
...
export class ProjectInitializationOrchestrator {
  ...
  private async runRequirements(project: Project) { const started = new Date().toISOString(); updateAgent(project.id, 'requirements', 'RUNNING', 'Analyzing project context'); await wait(650); const next = projectService.getProject(project.id)!; next.requirements = generatedRequirements(next); next.activity = [runRecord('requirements', 'Initialized project requirements', started, `${next.requirements.length} requirements identified`), ...next.activity]; projectService.saveProject(next); saveStatus(project.id, { ...readStatus(project.id), progress: 20, agents: readStatus(project.id).agents.map(item => item.agent === 'requirements' ? { ...item, status: 'COMPLETED', summary: `${next.requirements.length} requirements identified` } : item) }) }
  private async runArchitecture(project: Project) { const started = new Date().toISOString(); updateAgent(project.id, 'architecture', 'RUNNING', 'Designing system architecture'); await wait(650); const next = projectService.getProject(project.id)!; next.architecture = generatedArchitecture(next); next.activity = [runRecord('architecture', 'Generated system architecture', started, `${next.architecture.length} components mapped`), ...next.activity]; projectService.saveProject(next); saveStatus(project.id, { ...readStatus(project.id), progress: 40, agents: readStatus(project.id).agents.map(item => item.agent === 'architecture' ? { ...item, status: 'COMPLETED', summary: `${next.architecture.length} architecture components` } : item) }) }
  private async runPlanner(project: Project) { const started = new Date().toISOString(); updateAgent(project.id, 'planner', 'RUNNING', 'Building execution plan'); await wait(650); const next = projectService.getProject(project.id)!; next.tasks = generatedTasks(); next.activity = [runRecord('planner', 'Generated execution plan', started, `${next.tasks.length} execution tasks created`), ...next.activity]; projectService.saveProject(next); saveStatus(project.id, { ...readStatus(project.id), progress: 60, agents: readStatus(project.id).agents.map(item => item.agent === 'planner' ? { ...item, status: 'COMPLETED', summary: `${next.tasks.length} execution tasks created` } : item) }) }
  private async runReviewer(project: Project) { const started = new Date().toISOString(); updateAgent(project.id, 'reviewer', 'RUNNING', 'Reviewing engineering gaps'); await wait(650); const next = projectService.getProject(project.id)!; next.risks = generatedRisks(); next.activity = [runRecord('reviewer', 'Reviewed initialized engineering context', started, `${next.risks.length} engineering risks identified`), ...next.activity]; projectService.saveProject(next); saveStatus(project.id, { ...readStatus(project.id), progress: 80, agents: readStatus(project.id).agents.map(item => item.agent === 'reviewer' ? { ...item, status: 'COMPLETED', summary: `${next.risks.length} engineering risks identified` } : item) }) }
  private async runTesting(project: Project) { const started = new Date().toISOString(); updateAgent(project.id, 'testing', 'RUNNING', 'Generating verification coverage'); await wait(650); const next = projectService.getProject(project.id)!; next.tests = generatedTests(); next.completion = 0; next.nextAction = { title: 'Confirm success criteria', description: 'ProjectPilot found that the core objective needs measurable acceptance criteria before implementation begins.', priority: 'High', effort: '20–30 min', criteria: ['Primary success state is explicit', 'Failure behavior is documented', 'The first task has an observable outcome'] }; next.activity = [runRecord('testing', 'Generated verification strategy', started, `${next.tests.length} test cases created`), ...next.activity]; projectService.saveProject(next); saveStatus(project.id, { ...readStatus(project.id), progress: 100, agents: readStatus(project.id).agents.map(item => item.agent === 'testing' ? { ...item, status: 'COMPLETED', summary: `${next.tests.length} verification cases created` } : item) }) }
