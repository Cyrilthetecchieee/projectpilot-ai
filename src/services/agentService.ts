...
const run = async <T,>(value: T): Promise<T> => { await wait(); return value }
...
export const agentService = {
  ...
  async createTaskFromFinding(_projectId: string, risk: Risk): Promise<Task> { await wait(500); return { id: `T-${Date.now()}`, title: `Define fallback states for ${risk.title.toLowerCase()}`, milestone: 'Requirements & Design', priority: risk.severity, status: 'Pending', dependency: 'Safety state model', successCriteria: risk.recommendation } },
}
