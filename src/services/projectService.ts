...
import type { Project, Task, Requirement, TestCase, AgentRun, ArchitectureComponent, Risk } from '../types'
...
export const projectService = {
  ...
  addRequirement(requirement: Requirement, projectId: string): Project { const project = clone(this.getProject(projectId)!); project.requirements.push(requirement); return this.saveProject(project) },
  deleteRequirement(id: string, projectId: string): Project { const project = clone(this.getProject(projectId)!); project.requirements = project.requirements.filter(item => item.id !== id); return this.saveProject(project) },
  addComponent(component: ArchitectureComponent, projectId: string): Project { const project = clone(this.getProject(projectId)!); project.architecture.push(component); return this.saveProject(project) },
  updateComponent(id: string, component: Partial<ArchitectureComponent>, projectId: string): Project { const project = clone(this.getProject(projectId)!); project.architecture = project.architecture.map(item => item.id === id ? { ...item, ...component } : item); return this.saveProject(project) },
  deleteComponent(id: string, projectId: string): Project { const project = clone(this.getProject(projectId)!); project.architecture = project.architecture.filter(item => item.id !== id); return this.saveProject(project) },
  addRisk(risk: Risk, projectId: string): Project { const project = clone(this.getProject(projectId)!); project.risks.push(risk); return this.saveProject(project) },
  updateRisk(id: string, risk: Partial<Risk>, projectId: string): Project { const project = clone(this.getProject(projectId)!); project.risks = project.risks.map(item => item.id === id ? { ...item, ...risk } : item); return this.saveProject(project) },
  deleteRisk(id: string, projectId: string): Project { const project = clone(this.getProject(projectId)!); project.risks = project.risks.filter(item => item.id !== id); return this.saveProject(project) },
  ...
  addTestCase(test: TestCase, projectId: string): Project { const project = clone(this.getProject(projectId)!); project.tests.push(test); return this.saveProject(project) },
  deleteTestCase(id: string, projectId: string): Project { const project = clone(this.getProject(projectId)!); project.tests = project.tests.filter(item => item.id !== id); return this.saveProject(project) },
