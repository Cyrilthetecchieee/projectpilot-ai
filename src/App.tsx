...
function Dashboard({ project }: { project: Project }) { const navigate = useNavigate(); const completed = project.tasks.filter(t => t.status === 'Completed').length; return <><PageHeader eyebrow="PROJECT OVERVIEW" title={project.name} text={`${project.type} · ${project.stage}`} action={<Badge tone="lime">ACTIVE</Badge>} /><div className="overview-grid"><div className="completion-card panel"><div><span className="eyebrow">PROJECT COMPLETION</span><strong>{project.completion}%</strong></div><Progress value={project.completion} /><div className="completion-meta"><span>{completed} of {project.tasks.length} tasks complete</span><span>Updated just now</span></div></div><NextAction project={project} /></div><div className="section-label"><div><h2>Your AI Engineering Team</h2><span>AI reasoning powered by NVIDIA Nemotron</span></div><TechnologyBadge compact /></div><div className="metric-grid">{[['Requirements', `${project.requirements.length} Defined`, '2 Need Review', '/requirements', FileText], ['Architecture', `${project.architecture.length} Components`, '1 Interface Gap', '/architecture', Network], ['Tasks', `${completed} / ${project.tasks.length} Complete`, '4 In Progress', '/tasks', ClipboardCheck], ['Open Risks', `${project.risks.filter(r => !r.resolved).length}`, '1 Critical', '/risks', CircleAlert], ['Test Coverage', `${project.tests.length} Cases`, `${project.tests.filter(t => t.status === 'Pending').length} Pending`, '/testing', TestTube2], ['API Keys', 'Credentials Active', 'Manage Scopes', '/api-keys', Key]].map(([label, main, sub, path, I]) => <button className="metric-card" key={String(label)} onClick={() => navigate(`/project/${project.id}${String(path)}`)}><span className="metric-icon"><RenderIcon icon={I} /></span><small>{String(label)}</small><b>{String(main)}</b><span>{String(sub)}</span><ArrowRight size={15} /></button>)}</div><div className="split-grid"><div><div className="team-list">{[['Requirement Agent', 'Complete', FileText], ['Architecture Agent', 'Complete', Network], ['Planner Agent', 'Active', ClipboardCheck], ['Reviewer Agent', 'Attention Required', ShieldCheck], ['Test Agent', 'Ready', TestTube2]].map(([name, status, I]) => <div className="team-row" key={String(name)}><span className="icon-box small"><RenderIcon icon={I} size={16} /></span><b>{String(name)}</b><Badge tone={String(status).includes('Attention') ? 'high' : String(status).toLowerCase()}>{String(status)}</Badge></div>)}</div><span className="inference-caption">Inference via Nebius Token Factory</span></div><Findings project={project} /></div></> }
...
function AgentProgress({ title, steps, realProvider = false }: { title: string; steps: string[]; realProvider?: boolean }) { const agentName = title.startsWith('Architecture') ? 'Architecture Agent' : title.startsWith('Planner') ? 'Planner Agent' : 'Requirement Agent'; return <div className="agent-progress panel">{realProvider ? <div className="simulation-state"><span className="running-icon"><BrainCircuit size={18} /></span><div><strong>{agentName}</strong><span>Powered by NVIDIA Nemotron</span></div></div> : <AgentSimulationState agent={title.replace(' is analyzing your project...', '').replace(' is inspecting your project...', '').replace(' is building your execution plan...', '')} />}<div><p>Working through the project memory and engineering context.</p>{steps.map((step, i) => <div className="progress-step" key={step}><span className={i < 2 ? 'step-done' : ''}>{i < 2 ? <Check size={12} /> : <CircleDot size={12} />}</span>{step}</div>)}</div></div> }
...
function Tasks({ project, refresh }: { project: Project; refresh: () => void }) {
        ...
        activity: [
          {
            ...
            summary: `${result.tasks.length} tasks across ${result.milestones.length} milestones planned`,
          },
          ...current.activity,
        ],
      })
      setToast('Execution plan generated with technical dependencies.')
      refresh()
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : 'Execution plan could not be generated.')
    } finally {
      setRunning(false)
    }
  }

  const groups = project.milestones && project.milestones.length > 0
    ? project.milestones.map(m => m.title)
    : [...new Set(project.tasks.map(t => t.milestone))]

  const criticalPathSet = new Set(project.criticalPath || [])
  ...
  return (
    <>
      <PageHeader
        ...
        text="Milestones and tasks that turn the design into a working system."
        action={
          <div style={{ display: 'flex', gap: '8px' }}>
            <Button onClick={generate} icon={running ? RefreshCw : Sparkles}>
              {running ? 'Planning...' : 'Generate Execution Plan'}
            </Button>
            <Button secondary icon={Plus}>Add Task</Button>
          </div>
        }
      />
      {running && (
        <AgentProgress
          realProvider
          title="Planner Agent is building your execution plan..."
          steps={[
            'Validating requirements and architecture dependencies...',
            'Nemotron is constructing milestone hierarchy and tasks...',
            'Tracing components, interfaces, and architecture gaps...',
            'Calculating critical path and dependency graph...',
          ]}
        />
      )}
      {error && (
        <div className="agent-error panel">
          <strong>Planner Agent could not complete the plan.</strong>
          <span>{error}</span>
          <button className="btn btn-secondary" onClick={generate}>Retry</button>
        </div>
      )}
      {!running && (
        <>
          <div className="metrics-strip">
            <span><b>{project.tasks.length}</b> Tasks</span>
            <span><b>{project.tasks.filter(t => t.status === 'Completed').length}</b> Completed</span>
            <span><b>{project.tasks.filter(t => t.status === 'In Progress').length}</b> In Progress</span>
            <span><b>{project.tasks.filter(t => t.status === 'Pending').length}</b> Pending</span>
            {project.criticalPath && project.criticalPath.length > 0 && (
              <span><b>{project.criticalPath.length}</b> Critical Path</span>
            )}
          </div>
          {groups.length === 0 ? (
            <div className="architecture-empty">
              <ClipboardCheck size={24} />
              <h3>No execution plan generated yet.</h3>
              <p>Let the Planner Agent translate the requirements and architecture into actionable milestones and tasks.</p>
              <Button onClick={generate} icon={Sparkles}>Generate Execution Plan</Button>
            </div>
          ) : (
            <div className="milestones">
              {groups.map((group, i) => {
                const groupTasks = project.tasks.filter(t => t.milestone === group)
                const completedCount = groupTasks.filter(t => t.status === 'Completed').length
                const milestoneObj = project.milestones?.find(m => m.title === group)
                return (
                  <div className="milestone panel" key={group}>
                    <div className="milestone-head">
                      <div>
                        <span className="milestone-number">{String(i + 1).padStart(2, '0')}</span>
                        <div>
                          <h2>{group}</h2>
                          <span>
                            {completedCount} of {groupTasks.length} complete
                            {milestoneObj?.description ? ` · ${milestoneObj.description}` : ''}
                          </span>
                        </div>
                      </div>
                      <Progress value={groupTasks.length ? (completedCount / groupTasks.length) * 100 : 0} />
                    </div>
                    {groupTasks.map(task => {
                      const isCritical = task.is_critical_path || criticalPathSet.has(task.id)
                      return (
                        <div className={`task-row ${isCritical ? 'critical-path-task' : ''}`} key={task.id}>
                          <span className={`task-check ${task.status === 'Completed' ? 'checked' : ''}`}>
                            {task.status === 'Completed' && <Check size={13} />}
                          </span>
                          <div style={{ flex: 1 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                              <b>{task.title}</b>
                              {isCritical && <Badge tone="high">CRITICAL PATH</Badge>}
                              {task.estimated_effort && <span className="effort">{task.estimated_effort}</span>}
                            </div>
                            {task.description && (
                              <p style={{ margin: '4px 0 2px 0', fontSize: '13px', color: 'var(--text-muted)' }}>
                                {task.description}
                              </p>
                            )}
                            <span style={{ fontSize: '12px', color: 'var(--text-dim)' }}>
                              <b>{task.id}</b> · Criteria: {task.successCriteria}
                              {task.dependencies && task.dependencies.length > 0 && (
                                <> · <em>Depends on: {task.dependencies.join(', ')}</em></>
                              )}
                              {task.related_components && task.related_components.length > 0 && (
                                <> · <span>Components: {task.related_components.join(', ')}</span></>
                              )}
                            </span>
                          </div>
                          ...
                          <select
                            value={task.status}
                            onChange={e => {
                              projectService.updateTask(task.id, { status: e.target.value as TaskStatus }, project.id)
                              refresh()
                            }}
                          >
